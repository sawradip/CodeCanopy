...
class TreeGenerator:
    ...
    def generate(self, root_path: Path = None, focus_dirs: List[str] = None,
                ignore_patterns: List[str] = None, global_depth: int = None,
        ...
        self._generate_tree_recursive(
            root_path, lines, "", focus_paths, ignore_patterns, 
            global_depth, show_files, current_depth=0
        )
        
        return "\n".join(lines)
    
    def _generate_tree_recursive(self, path: Path, lines: List[str], prefix: str,
                                focus_paths: Set[Path], ignore_patterns: List[str],
                                global_depth: int, show_files: bool, current_depth: int = 0):
        ...
        files = [entry for entry in filtered_entries if entry.is_file()] if show_files else []
        ...
        for i, entry in enumerate(all_entries):
            ...
            if entry.is_dir():
                self._generate_tree_recursive(
                    entry, lines, next_prefix, focus_paths, ignore_patterns,
                    global_depth, show_files, current_depth + 1
                )
